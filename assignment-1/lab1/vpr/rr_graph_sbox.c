...
static struct s_ivec ***switch_block_conn=NULL;  
 ...
 * lists which tracks connect to which at each switch block.                 */
 ...
 int from_track, to_track;
    ...
    // allocate the switch block connection structure

// commented this out because I think it's already been allocated...
    ...
    for (to_side=0;to_side<=3;to_side++) {
       for (from_track=0;from_track<nodes_per_chan;from_track++) {
          ...
          if (from_side != to_side) {
             ...
             switch_block_conn[from_side][to_side][from_track].nelem = 0; // kaboom - why?
